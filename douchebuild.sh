...
# Script created by:
# Nicandros Nicolaou (nicandris)
# nicandris@gmail.com

# Changes for rpm distributions made by:
#	Michael H. Warfield   /\/\|=mhw=|\/\/
#	mhw@WittsEnd.com

# We need to know if this is an rpm or apt-get based distribution.
# Because some of the tools are "ported" we should not rely on the
# presence of apt-get, dpkg, yum, or rpm.  No really "good" solution
# so I'm taking the cowardly way out unless someone has a better idea
# and switching on the presence of /etc/redhat-release.
...
if [[ ${REDHAT} == true ]]
then
   # These RedHat dependencies are based on my x86_64 machine.  I may have
   # already had some other dev packages installed already.  If others are
   # needed, please let me know for addition!  Thanks!   mhw  /\/\|=mhw=|\/\/
   DEPENDENCIES="git gnupg flex bison gperf squashfs-tools zip curl pngcrush schedtool"
...
if [ `uname -m | sed 's/x86_//;s/i[3-6]86/32/'` != "32" ]; then
   if [[ ${REDHAT} == true ]]
   then
      MLIBDEPS="glibc-devel-2.14.1-5.i686"
      sudo yum install -y ${MLIBDEPS}
   else
